...
; file_load_va is the load address of the linker
file_load_va: equ 4096 * 80
; content_load_va is the load address of the main ELF executable with the data
content_load_va: equ 4096 * 40
...
entry_point:
  ...
  ; Move rbx past the argv array into envp
  mov eax, [rbx]
  lea rbx, [rbx+8*rax+8]

  ; Find the NULL in envp
.find_null_env:
  lea rbx, [rbx+8]
  mov rax, [rbx]
  cmp rax, 0
  ...
  ; Move rbx past the envp into auxv
  lea rbx, [rbx+8]
...
.found_entry_auxv:
  ; Read the value of AT_ENTRY
  mov rax, [rbx+8]
  cmp rax, file_load_va
  ; Loop if we are running the interpreter as an executable.
  jge .found_entry_auxv
...
compute_final_hash:
	mov	r15d, content_load_va + 0xD00
...
.LBB1_4:
  ...
  ; rax is already 0 assuming the write succeeded, so set the next syscall
